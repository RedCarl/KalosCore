...
public class BanList_A extends InventoryGUI {
    public BanList_A(Player player) {
        super(ColorParser.parse("&0"+ Data.SERVER_NAME+" / 物品封禁 &7(1)"), player, 6);
        ...
        for (int j = 0; j < 36; j++) {
            ...
            Button i = new Button(itemStacks.get(j), type -> {
                if (type.isLeftClick()){
                    if (player.hasPermission("kim.admin")){
                        if (itemStacks.get(finalJ).getItemMeta().getDisplayName().contains("禁止获取")){
                            player.closeInventory();
                            PokemonBanDataSQLReader.removeDrops(itemStacks.get(finalJ).getType().name());
