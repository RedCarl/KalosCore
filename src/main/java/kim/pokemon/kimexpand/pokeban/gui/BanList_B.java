...
public class BanList_B extends InventoryGUI {
    public BanList_B(Player player) {
        super(ColorParser.parse("&0"+ Data.SERVER_NAME+" / 物品封禁 &7(2)"), player, 6);
        ...
        if (player.hasPermission("kim.admin")){
            for (String item : PokemonBanDataSQLReader.getBanDrops()) {
                ItemStack itemStack = ItemFactoryAPI.getItemStack(Material.getMaterial(item),ColorParser.parse("&c[普通物品] / 禁止获取 &7(点击移除)"),ColorParser.parse("&7&o"+Material.getMaterial(item).name()));
                itemStacks.add(itemStack);
            }
        }else {
            for (String item : PokemonBanDataSQLReader.getBanDrops()) {
        ...
        for (int j = 0; j < 36; j++) {
            int finalJ = j;
            if(itemStacks.size()==36+j){
                break;
            }
            Button i = new Button(itemStacks.get(36+j), type -> {
                if (type.isLeftClick()){
                    if (player.hasPermission("kim.admin")){
                        if (itemStacks.get(36+finalJ).getItemMeta().getDisplayName().contains("禁止获取")){
                            player.closeInventory();
                            PokemonBanDataSQLReader.removeDrops(itemStacks.get(36+finalJ).getType().name());
                            player.sendMessage(ColorParser.parse("&8[&a&l!&8] &7成功将该物品从黑名单中移除."));
                        }else {
                            player.closeInventory();
                            player.sendMessage(ColorParser.parse("&8[&c&l!&8] &7很抱歉，您无法解除禁止合成的物品."));
                        }
                    }
                }
            });
