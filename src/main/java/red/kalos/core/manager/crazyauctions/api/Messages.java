...
public enum Messages {
    ...
    public String getMessage() {
        if (isList()) {
            if (exists()) {
                return Methods.color(convertList(FileManager.Files.MESSAGES.getFile().getStringList("Messages." + path)));
            } else {
                return Methods.color(convertList(getDefaultListMessage()));
            }
        } else {
            if (exists()) {
                return Methods.getPrefix(FileManager.Files.MESSAGES.getFile().getString("Messages." + path));
            } else {
    ...
    public String getMessage(HashMap<String, String> placeholders) {
        ...
        if (isList()) {
            if (exists()) {
                message = Methods.color(convertList(FileManager.Files.MESSAGES.getFile().getStringList("Messages." + path), placeholders));
            } else {
                message = Methods.color(convertList(getDefaultListMessage(), placeholders));
            }
        } else {
            if (exists()) {
                message = Methods.getPrefix(FileManager.Files.MESSAGES.getFile().getString("Messages." + path));
            } else {
    ...
    public String getMessageNoPrefix() {
        if (isList()) {
            ...
            if (exists()) {
                return Methods.color(FileManager.Files.MESSAGES.getFile().getString("Messages." + path));
            } else {
